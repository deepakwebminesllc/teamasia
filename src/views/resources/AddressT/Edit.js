...
const Edit = () => {
  ...
  const {id,name:Name,is_trashed:isTrashed} = location.state || {};
  ...
  async function apiCall() {
    try {
        ...
        const response = await fetch(`https://factory.teamasia.in/api/public/addresstypes/${id}`, {
            method: "PUT",
            headers: {
        ...
        if (response.status === 200) {
          navigate('/resources/address-types');
        } else {

...
const Edit = () => {
  ...
  const {id,name:DisplayName,status: Status,is_default_status:UseThisasdefaultstatusforOrders,is_notify: NotifyCustomersviaEmailandSMS,short_description:ShortDescription,sms_content:SmsContent,email_content:EmailContent,subject:Subject,is_trashed:isTrashed} = location.state || {}; // Default to an empty object if state is undefined
...
async function apiCall() {
  ...
  try {
      ...
      const response = await fetch(`https://factory.teamasia.in/api/public/orderstatuses/${id}`, {
          method: "PUT",
          headers: {

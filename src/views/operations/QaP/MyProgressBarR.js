...
const MyProgressBar = ({containerWidth, progressBarId,jumboId }) => {
  ...
  const updater =(width)=>{
   ...
   const faults = (FaultData)=>{
    ...
    const FaultSegmentArray = FaultData.map((fault)=>{
        return {
            start: Number(containerWidth) - (Number(fault.fault_start_point) + Number(fault.fault_length)),
            end : (Number(containerWidth) - (Number(fault.fault_start_point) + Number(fault.fault_length))) + Number(fault.fault_length),
            color: '#EB455F',
            code:fault.id
        }
      });
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const response = await fetch(`https://factory.teamasia.in/api/public/faultlogs/?jumbo_roll_id=0&small_roll_id=${jumboId}`, {
        method: 'GET', 
        headers: {

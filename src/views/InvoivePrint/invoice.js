...
const Invoice = () => {
    ...
    doc.setFont(undefined, 'normal');
    doc.text(`${addr.customer_name}`, 90, 70);
    ...
    doc.text('TMQ-L1 - missing', 195, 74, { align: 'right' });
    ...
    doc.autoTable({
        ...
        body: [
            ...
            ...data.autoInvoiceitems.map((item, index) => [
                ...
                `70m - missing `,
                item.tax_rate,
                (item.price * 70 * item.tax_rate)/100,
                (item.price * 70)
            ]),
...
function numberToWords(numAmount) {
    ...
    const generatePDF = (temp,OrderResult,addr,addr1) => {
        ...
        temp.autoInvoiceitems.forEach((item)=>{
                ...
                calc.totalTax += (item.price * 70 * item.tax_rate)/100;
                calc.subTotal += (item.price * 70)
